...
;
; PURPOSE:
;       This function calculates the FWHM at various wavelength positions
;       along the Spitzer/IRS low-resolution detectors.
;
; CATEGORY:
;       Astrophysics
;
; CALLING SEQUENCE:
;       Result = Jam_IRSLowFWHM(Wave0, Redshift)
;
; AUTHOR:
;       Jason A. Marshall
...
FUNCTION Jam_IRSLowFWHM, _wave0, redshift, Debug=debug
  ...
  limSL2 = [5.2, 7.4]
  idxSL2 = Where((waveIRS GE limSL2[0]) AND (waveIRS LT limSL2[1]))
  ...
  limLL1B = [19.5, 38.0]
  idxLL1B = Where((waveIRS GE limLL1B[0]) AND (waveIRS LE limLL1B[1]))
